...
public class Jugador_Controles : MonoBehaviour
{
    ...
    void DispararRayo()
    {
        ...
        {
            Vector3 puntoDeDisparo = camaraprimerapersona.transform.position + camaraprimerapersona.transform.forward * distanciaDesdeCamara;

            Ray ray = new Ray(puntoDeDisparo, camaraprimerapersona.transform.forward);
            RaycastHit hit;

            GameObject pro;
            pro = Instantiate(bala, puntoDeDisparo, transform.rotation);
